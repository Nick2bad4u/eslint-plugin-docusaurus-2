 ...
 * ESLint rule implementation for `no-conflicting-navbar-doc-item-props`.
 */
...
type MessageIds = "noConflictingNavbarDocItemProps";
    ...
    createTypedRule({
        create(context) {
            ...
            return {
                ObjectExpression(node: Readonly<TSESTree.ObjectExpression>) {
                    ...
                    const docIdProperty = findObjectPropertyByName(
                        node,
                        "docId"
                    );
        ...
        meta: {
            ...
            docs: {
                ...
                url: "https://nick2bad4u.github.io/eslint-plugin-docusaurus-2/docs/rules/no-conflicting-navbar-doc-item-props",
            },
            fixable: "code",
            messages: {
                noConflictingNavbarDocItemProps:
                    'Docusaurus navbar items with `type: "doc"` should not mix in direct-link props such as `to`, `href`, or `html`. Keep the `docId`-based shape only.',
            },
            schema: [],
            type: "problem",
        },
        name: "no-conflicting-navbar-doc-item-props",
    });
