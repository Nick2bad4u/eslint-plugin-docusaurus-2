...
description: "🤖🤖 Use this prompt to scaffold a brand-new ESLint plugin repository from my modern template. IMPORTANT: treat the previous template plugin only as a structural and quality guide, never as rule content to keep, rename, or adapt unless I explicitly ask for a similar rule."
...
The current repository may have been scaffolded from my template plugin, but that does **not** mean you should keep, port, rename, reinterpret, or adapt the template-specific rules into the new plugin unless I explicitly ask for a similar rule.
...
- do not keep template-plugin rules as starter rules for the new plugin
- do not rename template-specific rule implementations to fit the new plugin
- do not copy template rule docs/examples/options into the new plugin as filler
...
1. The root of this repository has been scaffolded using my template repo. All root `package.json` dependencies & devDependencies are already installed unless the user says otherwise.
...
6. **Establish the new plugin identity:**
   ...
   - Remove or update all stale references to the previous template plugin, namespace, and helper-library examples.
...
15. **Final check:**
   ...
   - there are no leftover template-plugin or helper-library references
...
2. Is this **rule behavior/content/docs/examples**? If yes, it must come from the user's explicit requirements, not from the previous template plugin.
...
A. The repository is fully re-identified as the new plugin rather than the previous template plugin.
...
F. The entire project matches the layout, doc standards, and coding standards of my template **without importing unrelated template-specific rule content**.
...
- **Do NOT** keep, rename, or adapt any template-plugin rule implementations, tests, or docs unless I explicitly requested an equivalent rule.
...
1. **Inventory template-specific rule/docs/test artifacts and remove them surgically.**
...
**If you are ever unsure, default to preserving infrastructure and performing targeted removals of obsolete template-specific artifacts only.**
