...
description: "🤖🤖 Use this prompt to bootstrap a new ESLint plugin repository by migrating the source plugin into my modern template structure. IMPORTANT: treat the previous template plugin only as a structural and quality guide, never as rule content to port or convert unless the source plugin already has an equivalent rule."
...
The current repository may have been scaffolded from my template plugin, but that does **not** mean you should port, rename, reinterpret, or adapt the template-specific rules into the new plugin.
...
- do not convert template-plugin rules into rules for the new plugin
- do not rename template-specific rule implementations to fit the new plugin
- do not copy template rule docs/examples/options into the new plugin unless they are genuinely part of the source plugin being migrated
...
2. The root of this repository has been scaffolded using my template repo. All root `package.json` dependencies & devDependencies are already installed.
...
8. **Modernize:** Rewrite all migrated rules in TypeScript and update them to modern ESLint 10 plugin standards. Preserve the source plugin's intent; do not replace it with template-plugin rule ideas.
...
2. Is this **rule behavior/content/docs/examples**? If yes, it must come from the source plugin or explicit user direction, not from the previous template plugin.
...
D. The entire project matches the layout, doc standards, and coding standards of my template **without importing unrelated template-specific rule content**.
...
## 🚨 ABSOLUTE RULE: REMOVE ALL TEMPLATE-SPECIFIC CONTENT

- **Do NOT** leave any template-plugin rule code, documentation, or examples in the new repository.
- **Do NOT** create new folders like `src/[New-Plugin-Name]/rules` or `src/[New-Plugin-Name]`.
- **Do NOT** keep, rename, or adapt any template-plugin rule implementations, tests, or docs unless the source plugin has a direct equivalent you are intentionally migrating.
- **Do NOT** leave any references to the previous template plugin, helper-library examples, or their rules/options/examples anywhere in the codebase, docs, or tests.
...
1. **Inventory template-specific rule code, docs, and tests and remove them surgically during migration.**
	- Do not do blanket wipe-first deletion.
	- Keep shared infrastructure and adapt it in place.
2. **Migrate only the rules, tests, and docs that exist in the source plugin** (in `./[SOURCE_PLUGIN_FOLDER]`). Do not invent new rules or copy anything from the previous template plugin unless the source plugin has a direct equivalent.
3. **Update all configs, scripts, and docs** to remove any references to the previous template plugin, helper-library examples, or their rules.
...
- The final repo must have **zero** template-plugin rule code, docs, or leftover helper-library references.
...
**If you are ever unsure, default to preserving infrastructure and only removing clearly obsolete template-specific rule artifacts once replacement coverage exists.**
